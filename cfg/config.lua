...
cfg.leite= {
  ["lapte"] = {
    ...
    choices = function(args)
	  local menu = {}
      menu["Bea"] = {function(player,choice)
        local user_id = vRP.getUserId({player})
        if user_id ~= nil then
          if vRP.tryGetInventoryItem({user_id,"lapte",1,false}) then
            vRP.varyThirst({user_id,-sede_leite})
            vRPclient.notify(player,{"~b~Bei Lapte"})
            local seq = {
