...
vRPl = {}
Lserver = Tunnel.getInterface("vrp_vacar","vrp_vacar")
vRPl = Tunnel.getInterface("vrp_vacar","vrp_vacar")
    ...
    while true do
		...
		for k,v in pairs(cfg.vacas)do
			...
			if(Vdist(pos.x, pos.y, pos.z, v[1], v[2], v[3]) < 2.0)then
				DrawText3D(v[1], v[2], v[3], "~w~Apasa ~g~[E] ~w~pentru a ~y~Mulge~w~ vaca.\nApasa ~g~[F]~w~ pentru a ~r~Ucide~w~ vaca.",255,255,255)
				if IsControlJustPressed(1, 38) and not coletando then
					if GetEntityHealth(vacas[k]) > 50 then
						...
						if(Vdist(x, y, z, v[1], v[2], v[3]) < 4.0)then
							Lserver.coletarLeite({true,k})
							coletando = true
							vaca = k
						else
							vRP.notify({"~r~Vaca nu este in loc ul potrivit"})
						end
					else
						vRP.notify({"Nu este posibil sa obtineti lapte de la o vaca moarta"})
					end
				end
				if IsControlJustPressed(1, 49) and not coletando then
					x, y, z = table.unpack(GetEntityCoords(vacas[k], true))
					if(Vdist(x, y, z, v[1], v[2], v[3]) < 4.0)then
						Lserver.coletarLeite({false,k})
						-- if haspermission then
							FreezeEntityPosition(vacas[k], false)
							Lserver.conduzir({k})
						-- else
							-- vRP.notify({"~r~Não possui permissão."})
						-- end
					else
						vRP.notify({"~r~Vaca nu este in loc ul potrivit "})
					end
...
Citizen.CreateThread(function()
	while true do
		...
		if(Vdist(pos.x, pos.y, pos.z, cfg.abatedouro[1], cfg.abatedouro[2], cfg.abatedouro[3]) < 10.0)then
			DrawText3D(cfg.abatedouro[1], cfg.abatedouro[2], cfg.abatedouro[3], "~r~Abator",255,255,255)
		end
	end
end)
...
AddEventHandler("vrp:abater", function(k)
	...
	if cfg.follow_player then
		vRP.notify({"Pentru a primii carne si lapte: \nDu-te cu vaca la ~r~Abator"})
	end
	while true do
		if cfg.follow_player then
		...
		if (GetEntityHealth(vacas[k])) == 0 then
			vRP.notify("Vaca ~r~ a murit ~w~ în afara abatorului")
			return
		end
		x, y, z = table.unpack(GetEntityCoords(vacas[k], true))
...
Citizen.CreateThread(function()
	...
	--while true do
		...
		for k,v in pairs(cfg.vacas)do
			...
			vacas[k] = CreatePed(4, "a_c_cow", v[1], v[2], v[3]-1, 0.0, false, false)
